...
app.post('/regUser',(req:any,res:any)=>{
    let rut=req.body.rut;
    let uname=req.body.uname;
    let password=req.body.password;
    let celular=req.body.celular;
    pool.query("INSERT INTO public.usuarios (rut,uname, password, celular) VALUES ($1,$2,crypt($3, gen_salt('bf')),$4)",[rut,uname,password,celular],(req1:any,resultados:any)=>{
...
app.post('/LogIn', bodyParser.json(), function(request:any, response:any)
{
	...
	if (uname && password){
		pool.query("SELECT * FROM public.usuarios WHERE uname = $1 and password = crypt($2, password)", [uname, password], async function(error:any, results:any, fields:any){
   ...
   console.log(Object.entries(products))
    ...
    pool.query('SELECT * FROM public."usuarioMesaBoleta" WHERE "idMesa" = $1 ORDER BY "idBoleta" DESC;',[mesa],(req1:any,resultados:any)=>{
        ...
        for (let product of Object.entries(products)){
            pool.query('INSERT INTO public.detalle("idBoleta", "idProducto", cant) VALUES ($1, $2, $3);',[boleta, product[0], product[1]],(req1:any,resultados:any)=>{
...
app.post('/postDiscount',(req:any,res:any)=>
{
    let porcentaje= req.body.porcentaje;
    let mincompra= req.body.mincompra ;
    let descripcion= req.body.descripcion ;
    let status= req.body.status ;
    pool.query("INSERT INTO public.descuentos (porcentaje,mincompra, descripcion, status) VALUES ($1,$2,$3,$4)",[porcentaje,mincompra,descripcion,status],(req1:any,resultados:any)=>{
        res.status(201).send(resultados);
        console.log(req1);
    });
});
...
app.get('/getMesas',(request:any, response:any) => {
    pool.query("SELECT * FROM public.mesas ORDER BY id ASC", function(error:any, results:any, fields:any){
